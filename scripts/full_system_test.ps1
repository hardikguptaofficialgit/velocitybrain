...
try {
    ...
    Invoke-Step -Name "CLI smoke" -Action {
        ...
        Invoke-CheckedCommand -FilePath $pythonExe -Arguments @('-m', 'src.cli', 'ingest', '--source', 'note', '--content', 'Full test note for Jane Doe at Acme')
        Invoke-CheckedCommand -FilePath $pythonExe -Arguments @('-m', 'src.cli', 'query', 'What do I know about Jane Doe?')
        Invoke-CheckedCommand -FilePath $pythonExe -Arguments @('-m', 'src.cli', 'run', 'Prepare me for a meeting with Jane Doe tomorrow')
